...
export function initSidebar() {
    ...
    const openPathsSet = new Set(
        JSON.parse(localStorage.getItem(OPEN_DIRS_STORAGE_KEY) || '[]')
    );
...
export function initTabs(easyMDE) {
    ...
    tabs.forEach(tab => {
        tab.addEventListener('click', () => {
            ...
            document.querySelectorAll('.tools-content > div').forEach(div => div.classList.remove('active'));
    ...
    // --- CASE B: READ HISTORY (On Dashboard) ---
        ...
        if (historyGrid) {
            ...
            if (history.length > 0) {
                ...
                const historyToggles = Array.from(document.querySelectorAll('a.section-link'))
                    .filter(a => /history/i.test(a.textContent || ''));
