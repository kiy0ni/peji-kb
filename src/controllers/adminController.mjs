...
export const getAdminDashboard = (req, res) => {
  ...
  const userWebhooks = selectedUser
    ? db
...
export const revokeUserKey = (req, res) => {
  try {
    db.prepare('UPDATE api_keys SET active = 0 WHERE id = ? AND user_id = ?').run(
      req.params.keyId,
      req.params.id
    );
  } catch (e) {
...
export const deleteUserWebhook = (req, res) => {
  try {
    db.prepare('DELETE FROM webhooks WHERE id = ? AND user_id = ?').run(
      req.params.hookId,
      req.params.id
    );
  } catch (e) {
