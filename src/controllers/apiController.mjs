...
export const saveSnippets = (req, res) => {
    ...
    try {
        ...
        const transaction = db.transaction(() => {
            ...
            const insert = db.prepare('INSERT INTO snippets (path, user_id, code, created_at) VALUES (?, ?, ?, ?)');
...
export const getCombinedData = (req, res) => {
    ...
    try {
        const noteRow = db.prepare('SELECT content FROM notes WHERE path = ? AND user_id = ?').get(coursePath, userId);
        const snippets = db.prepare('SELECT code, created_at as timestamp FROM snippets WHERE path = ? AND user_id = ? ORDER BY created_at ASC').all(coursePath, userId);
...
export const saveCombinedData = (req, res) => {
    ...
    const saveTransaction = db.transaction((cPath, d) => {
        ...
        if (d.snippets) {
            ...
            const insert = db.prepare('INSERT INTO snippets (path, user_id, code, created_at) VALUES (?, ?, ?, ?)');
            d.snippets.forEach(s => insert.run(cPath, userId, s.code, s.timestamp || new Date().toISOString()));
        }
    });
    ...
    try {
        if (action === 'start') {
            ...
            if (row?.id) {
                ...
                db.prepare(`UPDATE reading_sessions SET ended_at = CURRENT_TIMESTAMP WHERE id = ?`).run(row.id);
...
export const trackSite = (req, res) => {
    ...
    try {
        if (action === 'start') {
            db.prepare(`UPDATE site_sessions SET ended_at = CURRENT_TIMESTAMP WHERE user_id = ? AND ended_at IS NULL`).run(userId);
            db.prepare(`INSERT INTO site_sessions (user_id, last_route, started_at) VALUES (?, ?, CURRENT_TIMESTAMP)`).run(userId, route || null);
            dispatchWebhook(userId, 'site.started', { route });

        } else if (action === 'ping' || action === 'stop') {
            const row = db.prepare(`SELECT id FROM site_sessions WHERE user_id = ? AND ended_at IS NULL ORDER BY started_at DESC LIMIT 1`).get(userId);
...
export const getWebhooks = (req, res) => {
    const userId = getUserId(req);
    const rows = db.prepare('SELECT id, url, events, active, created_at FROM webhooks WHERE user_id = ?').all(userId);
    res.json({ items: rows });
};
...
export const createWebhook = (req, res) => {
    ...
    try {
        ...
        return isBrowserRequest ? res.redirect('/settings') : jsonError(res, 400, 'Invalid URL format', 'bad_url'); 
    }
