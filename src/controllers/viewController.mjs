...
export const getDashboard = (req, res) => {
    ...
    const coursesReadRow = db.prepare(`
    ...
    const productivity = siteSeconds > 0 
        ? Math.min(100, Math.round((readingSeconds / siteSeconds) * 100)) 
        : (readingSeconds > 0 ? 100 : 0);
...
export const getFileViewer = (req, res) => {
    ...
    const favEntry = db.prepare('SELECT path FROM favorites WHERE path = ? AND user_id = ?')
        .get(safeRel, userId);
        
    // Get list of all favorites for the sidebar
    const favorites = db.prepare('SELECT * FROM favorites WHERE user_id = ? ORDER BY created_at DESC')
        .all(userId);
...
export const getSettings = (req, res) => {
    ...
    res.render('layout', {
        ...
        webhooks: db.prepare('SELECT id, url, events, active, created_at FROM webhooks WHERE user_id = ?')
            .all(req.session.userId),
        lastCreatedKey
    });
};